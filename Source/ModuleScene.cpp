...
UpdateResult ModuleScene::Update()
{
	...
	if (currentStep == FADE_IN)		//Si hay FADE IN, se ejecuta el Update HASTA QUE EL RECTANGULO NEGRO tenga opacidad máxima
	{
		currentFrame++;
		currentScene->Update();
		if (currentFrame >= maxFrames)	//Si el rectángulo negro tiene opacidad máxima, cambiamos de escena e incializamos la siguiente
		{
